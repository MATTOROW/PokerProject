...
public class ConnectToRoomService {
    ...
    public String connectToRoom(String roomId, String token) throws ClientException {
        ...
        if (response.getType() == ClientMessageType.CONNECT_TO_ROOM_RESPONSE) {
            ...
            GameServerMessage gameServerResponse = client.sendMessageToGameServer(GameServerMessageUtils.createMessage(
                    GameMessageType.CONNECT_TO_ROOM_REQUEST,
                    "%s;%s".formatted(roomId, token).getBytes()
                    )
            );
            if (gameServerResponse.getType() == GameMessageType.CONNECT_TO_ROOM_RESPONSE) {
                return new String(gameServerResponse.getData());
            } else {
                throw new ClientException(new String(gameServerResponse.getData()));
            }
        } else {
